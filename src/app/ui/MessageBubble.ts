...
export class MessageBubble extends Container {
  ...
  constructor(
    ...
    const profilePath = MENTOR_PROFILES[senderName.toLowerCase()] || "main/mentors/0.JPG";
    ...
    this.messageText.y = opts.padding + Math.max(this.senderName.height, opts.profileSize) + 4;
    ...
    const contentHeight = Math.max(this.senderName.height, opts.profileSize) + this.messageText.height + 8;
  ...
  public setSenderName(name: string): void {
    ...
    const profilePath = MENTOR_PROFILES[name.toLowerCase()] || "main/mentors/0.JPG";
    this.profilePicture.texture = Texture.from(profilePath);
  }
